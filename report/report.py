...
with open(file_src) as f:
    for i in f:
        ...
        # if cdr[index[4]] == "0" and len(cdr[index[1]]) == 11 and tt1 <= int(cdr[index[0]]) <= tt2: # в период времени
        if cdr[index[1]] == "84991107520" and (cdr[index[2]] == "41114" or cdr[index[2]] == "41148"):
            # cdr[index[0]] = time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime(int(cdr[index[0]])))
            cdr[index[0]] = time.strftime("%d %b %Y %H:%M:%S", time.localtime(int(cdr[index[0]])))
            for x in index:
                ff.write(cdr[x] + " \t\t\t")
            ff.write("\n")
        # j += 1
